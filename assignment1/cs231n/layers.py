...
def affine_forward(x, w, b):
    ...
    输入的x的形状为(N, d_1, ..., d_k)包含了一个大小为N的minibatch的样本，每一样本x[i]
    的形状为(d_1, ..., d_k).我们将会把数据维度转化到一个向量中 D = d_1 * ... * d_k,
    并且把它映射到维度M的输出向量。
...
def affine_backward(dout, cache):
    ...
    dx = np.dot(dout, w.T).reshape(shape=x.shape)      # (N, D1,d2,d3)=(N,M) * (M,d1,d2,d3)
    dw = np.dot(x.T, dout).reshape()                                      # (D,M) = (d1,d2,d3,N)*(N,M)
